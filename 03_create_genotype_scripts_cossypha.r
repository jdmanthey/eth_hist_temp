...
##############    START OPTIONS     #####################################
	...
	project_directory <- "/lustre/scratch/jmanthey/07_ethiopia_historical/cos_sem"
	directory_name <- "11_genotype_cossypha"
	reference_genome_location <- "/home/jmanthey/hist_references/Tae_gut/GCF_003957565.2_bTaeGut1.4.pri_genomic.fna"
	cluster <- "quanah"
	output_name <- "cossypha"
	popmap <- "popmap_cos.txt"
	individuals <- read.table(popmap, sep="\t")
	individuals[,1] <- as.character(individuals[,1])
	faidx <- read.table("GCF_003957565.2_bTaeGut1.4.pri_genomic.fna.fai", stringsAsFactors=F)
